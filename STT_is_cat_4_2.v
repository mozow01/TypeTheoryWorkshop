...
| E_exp_ax : forall {x y z} (g : Hom_STT (Cnj x y) z),
  ...
  | E_unique_exp : forall {x y z} (h : Hom_STT x (Imp y z)),
      STT_equiv (proj1_sig (Lam_STT x y z (Compose_STT (Expapp_STT y z) (Prodmor_STT (Cnj x y) (Imp y z) y (Compose_STT h (First_STT x y)) (Compose_STT (Id_STT y) (Second_STT x y)))))) (proj1_sig h).
...
Lemma unique_prod_eta_STT : forall {x y z} (g : Hom_STT z (Cnj x y)),
  EqMor_STT (Prodmor_STT z x y (Compose_STT (First_STT x y) g) (Compose_STT (Second_STT x y) g)) g.
Proof.
  intros x y z g.
...
Lemma Prodmor_STT_congr : forall {x y z} {f f' : Hom_STT x y} {g g' : Hom_STT x z},
  EqMor_STT f f' ->
  EqMor_STT g g' ->
  EqMor_STT (Prodmor_STT x y z f g) (Prodmor_STT x y z f' g').
Proof.
  intros x y z f f' g g' Hf Hg.
  unfold EqMor_STT, Prodmor_STT, Prodmor_STT_term; simpl.
  apply E_lam.
  apply E_cnj.
  - apply E_app; [apply Hf | reflexivity].
  - apply E_app; [apply Hg | reflexivity].
Defined.


Lemma unique_prod_STT : forall {x y z} (f : Hom_STT x y) (g : Hom_STT x z) (h : Hom_STT x (Cnj y z)),
  (EqMor_STT (Compose_STT (First_STT y z) h) f) ->
  (EqMor_STT (Compose_STT (Second_STT y z) h) g) ->
  EqMor_STT h (Prodmor_STT x y z f g).
Proof.
...
Lemma exp_ax_STT : forall {x y z} (g : Hom_STT (Cnj x y) z),
  EqMor_STT (Compose_STT (Expapp_STT y z)
     (Prodmor_STT (Cnj x y) (Imp y z) y
        (Compose_STT (Lam_STT x y z g) (First_STT x y))
        (Compose_STT (Id_STT y) (Second_STT x y)))) g.
Proof. intros. unfold EqMor_STT. simpl. apply E_exp_ax. Defined.

Lemma unique_exp_STT_eta : forall {x y z} (h : Hom_STT x (Imp y z)),
  EqMor_STT (Lam_STT x y z (Compose_STT (Expapp_STT y z)
     (Prodmor_STT (Cnj x y) (Imp y z) y
        (Compose_STT h (First_STT x y))
        (Compose_STT (Id_STT y) (Second_STT x y))))) h.
Proof. intros. unfold EqMor_STT. simpl. apply E_unique_exp. Defined.

Instance STT_is_a_CCC : CartClosedCat STT_as_a_Cat.
Proof.
  apply CartClosedCat_mk with
    (Top_obj := Top)
    (Top_mor := fun x => Topmor_STT x)
    (Prod_obj := Cnj)
  ...
  - (* unique_top *)
    intros. apply unique_top_STT.
  - (* prod_ax *)
    intros. split; [apply prod_ax_1_STT | apply prod_ax_2_STT].
  - (* unique_prod *)
    intros. destruct H as [Hf Hg]. apply (unique_prod_STT f g h); assumption.
  - (* exp_ax *)
    intros. apply exp_ax_STT.
  - (* unique_exp *)
    intros. apply unique_exp_STT_eta.
Defined.

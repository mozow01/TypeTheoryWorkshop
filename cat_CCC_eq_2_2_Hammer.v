...
-intros. rewrite unique_exp. reflexivity. *)
Defined. 
...
Proof. 
  intros. 
  assert ( H1:   pr_1 ∘ (g Π Id A ∘ (f Π Id A)) = pr_1 ∘ ((g ∘ f) Π Id A)). { rewrite prod_ax_1; rewrite assoc; rewrite prod_ax_1; rewrite <- assoc; rewrite prod_ax_1; rewrite assoc; reflexivity. }
  assert ( H2:  pr_2 ∘ (g Π Id A ∘ (f Π Id A)) = pr_2 ∘ ((g ∘ f) Π Id A) ). { rewrite prod_ax_2; rewrite assoc; rewrite prod_ax_2; symmetry; rewrite <- assoc; rewrite prod_ax_2; rewrite id_2; rewrite id_2; reflexivity. }
  symmetry. 
  apply unique_prod. 
  split.
  all: try rewrite H1; try rewrite H2; try rewrite prod_ax_1; try rewrite prod_ax_2; reflexivity.
Defined.
...
(F_Hom := fun x y f => Lam (f ∘ Ev)).
  - intros. (*identity rule for functor*)
    rewrite id_2. 
    assert (H: Lam (Ev ∘ (Prod_mor (Id (Exp_obj x A) ∘ pr_1) ((Id A) ∘ pr_2))) = Id (Exp_obj x A) ). { apply unique_exp. }
    rewrite <- H. 
    rewrite id_2.
    rewrite id_2. 
    rewrite <- pr_and_id. 
    rewrite id_1. 
    reflexivity.
  - intros. (*composition rule for functor*)
    assert (H: g ∘ (Ev ∘  ( Prod_mor (Lam ( f ∘ Ev ) ∘ pr_1) (Id A ∘ pr_2))) = g ∘ (f ∘ Ev )). { rewrite exp_ax; reflexivity. }
    rewrite <- assoc. 
    rewrite <- H. 
    rewrite assoc. 
    assert (H1 : (g ∘ Ev ∘ (Lam (f ∘ Ev) Π Id A)) = Ev ∘ ( Lam ( g ∘ Ev) Π Id A) ∘ (Lam ( f ∘ Ev) Π Id A )). { rewrite exp_ax; reflexivity. }
    rewrite  H1. 
    rewrite <- assoc. 
    rewrite prod_id. 
    rewrite unique_exp. 
...
Instance RightAdjFunc (C D : Category) (F : CovariantFunctor D C) (FLAdj : IsLeftAdjoint C D F) : CovariantFunctor C D.
Proof.
...
Class Monad {Cat : Category} (M : CovariantFunctor Cat Cat) := mk_Monad_Bind_Style {
